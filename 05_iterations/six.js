...
const values = coding.forEach( (item)=>{  //foreach loop does not return any data so we use filter() more often
    // console.log(item);
    return item
})
...
// const newNums = myNums.filter( (num)=> num>5) //filter return value by checking the statement
// console.log(newNums);
